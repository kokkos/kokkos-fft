...
inline void _exec(rocfft_plan& plan, float* idata, std::complex<float>* odata,
           [[maybe_unused]] int direction, const rocfft_execution_info& execution_info) {
  rocfft_status status = rocfft_execute(plan, (void**)&idata, (void**)&odata, execution_info);
...
inline void _exec(rocfft_plan& plan, std::complex<float>* idata, std::complex<float>* odata, 
           [[maybe_unused]] int direction, const rocfft_execution_info& execution_info) {
  rocfft_status status = rocfft_execute(plan, (void**)&idata, (void**)&odata, execution_info);
  if(status != rocfft_status_success)
...
inline void _exec(rocfft_plan& plan, std::complex<double>* idata, std::complex<double>* odata, 
           [[maybe_unused]] int direction, const rocfft_execution_info& execution_info) {
  rocfft_status status = rocfft_execute(plan, (void**)&idata, (void**)&odata, execution_info);
  if(status != rocfft_status_success)
