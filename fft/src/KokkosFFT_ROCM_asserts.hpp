...
#define KOKKOSFFT_CHECK_ROCFFT_CALL(call)                       \
  KokkosFFT::Impl::check_rocfft_call(                           \
...
#else
#define KOKKOSFFT_CHECK_ROCFFT_CALL(call)                             \
  KokkosFFT::Impl::check_rocfft_call(call, #call, __FILE__, __LINE__, \
                                     __FUNCTION__)
#endif
...
inline void check_rocfft_call(rocfft_status command, const char* command_name,
  ...
  if (command) {
    ...
    ss << "\n"
       ...
       << rocfft_result_to_string(command) << ")\n";
