...
auto get_in_out_array_type(TransformType type, [[maybe_unused]] Direction direction) {
  ...
  return std::tuple<rocfft_array_type, rocfft_array_type, rocfft_transform_type>({in_array_type, out_array_type, fft_direction});
};

template <typename ValueType>
rocfft_precision get_in_out_array_type() {
  return std::is_same_v<KokkosFFT::Impl::real_type_t<ValueType>, float> ? rocfft_precision_single : rocfft_precision_double;
}

// Helper to convert the integer type of vectors
template <typename InType, typename OutType>
auto convert_int_type_and_reverse(std::vector<InType>& in) -> std::vector<OutType> {
...
// (n0) -> (1)
template <typename InType, typename OutType>
auto compute_strides(const std::vector<InType>& extents) -> std::vector<OutType> {
  ...
  for(std::size_t i=1; i<reversed_extents.size(); i++) {
    out.push_back( static_cast<OutType>(reversed_extents.at(i-1)) * out.at(i-1) );
  } 
...
auto _create(const ExecutionSpace& exec_space, std::unique_ptr<PlanType>& plan,
             const InViewType& in, const OutViewType& out, BufferViewType& buffer, 
             InfoType& execution_info, [[maybe_unused]] Direction direction, axis_type<fft_rank> axes) {
  ...
  auto _fft_extents = convert_int_type_and_reverse<int, std::size_t>(fft_extents);
  ...
  auto [in_array_type, out_array_type, fft_direction] = get_in_out_array_type(type, direction);
  ...
  if(workbuffersize > 0) {
    ...
    status = rocfft_execution_info_set_work_buffer(execution_info,
                                                   (void *)buffer.data(),
                                                   workbuffersize);
    if(status != rocfft_status_success)
