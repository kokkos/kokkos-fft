...
namespace KokkosFFT {
  ...
  void fft(const ExecutionSpace& exec_space,
    ...
    if(n) {
      ...
      auto modified_shape = KokkosFFT::Impl::get_modified_shape(in, shape_type<1>({_n}));
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, typename PlanType>
  void fft(const ExecutionSpace& exec_space,
           const InViewType& in,
           OutViewType& out,
           const PlanType& plan,
    ...
    plan.template good<ExecutionSpace, InViewType, OutViewType>(KokkosFFT::Impl::Direction::Forward, axis_type<1>{axis});
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType>
  void ifft(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            int axis=-1,
  ...
  void ifft(const ExecutionSpace& exec_space,
    ...
    plan.template good<ExecutionSpace, InViewType, OutViewType>(KokkosFFT::Impl::Direction::Backward, axis_type<1>{axis});
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType>
  void rfft(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            int axis=-1,
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType>
  void irfft(const ExecutionSpace& exec_space,
             const InViewType& in,
             OutViewType& out,
             KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
             int axis=-1,
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType>
  void hfft(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            int axis=-1,
    ...
    // [TO DO]
    // allow real type as input, need to obtain complex view type from in view type
    using in_value_type = typename InViewType::non_const_value_type;
    using out_value_type = typename OutViewType::non_const_value_type;
    static_assert(KokkosFFT::Impl::is_complex<in_value_type>::value,
    ...
    //using ComplexViewType = typename KokkosFFT::Impl::complex_view_type<ExecutionSpace, InViewType>::type;
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType>
  void ihfft(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            int axis=-1,
  ...
  void fft2(const ExecutionSpace& exec_space,
            const InViewType& in, OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            axis_type<2> axes={-2, -1},
            shape_type<DIM> s={0}) {
    static_assert(Kokkos::is_view<InViewType>::value,
  ...
  // 2D FFT
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, typename PlanType, std::size_t DIM = 1>
  void fft2(const ExecutionSpace& exec_space,
            const InViewType& in, OutViewType& out,
            const PlanType& plan,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
    ...
    plan.template good<ExecutionSpace, InViewType, OutViewType>(KokkosFFT::Impl::Direction::Forward, axes);
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM = 1>
  void ifft2(const ExecutionSpace& exec_space,
             const InViewType& in,
             OutViewType& out,
             KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
             axis_type<2> axes={-2, -1},
  ...
  void ifft2(const ExecutionSpace& exec_space,
    ...
    plan.template good<ExecutionSpace, InViewType, OutViewType>(KokkosFFT::Impl::Direction::Backward, axes);
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM = 1>
  void rfft2(const ExecutionSpace& exec_space,
             const InViewType& in,
             OutViewType& out,
             KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
             axis_type<2> axes={-2, -1},
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM = 1>
  void irfft2(const ExecutionSpace& exec_space,
              const InViewType& in,
              OutViewType& out,
              KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
              axis_type<2> axes={-2, -1},
  ...
  // ND FFT
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM = 1>
  void fftn(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
            shape_type<DIM> s={0}) {
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM1=1, std::size_t DIM2=1>
  void fftn(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            axis_type<DIM1> axes,
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, typename PlanType, std::size_t DIM1=1, std::size_t DIM2=1>
  void fftn(const ExecutionSpace& exec_space,
            const InViewType& in,
            OutViewType& out,
            const PlanType& plan,
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM=1>
  void ifftn(const ExecutionSpace& exec_space,
             const InViewType& in,
             OutViewType& out,
             KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
             shape_type<DIM> s={0}) {
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM=1>
  void rfftn(const ExecutionSpace& exec_space,
             const InViewType& in,
             OutViewType& out,
             KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
             shape_type<DIM> s={0}) {
  ...
  template <typename ExecutionSpace, typename InViewType, typename OutViewType, std::size_t DIM=1>
  void irfftn(const ExecutionSpace& exec_space,
              const InViewType& in,
              OutViewType& out,
              KokkosFFT::Normalization norm=KokkosFFT::Normalization::BACKWARD,
              shape_type<DIM> s={0}) {
