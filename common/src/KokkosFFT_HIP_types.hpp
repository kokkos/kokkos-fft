...
namespace Impl {
  ...
  using FFTDirectionType = int;
    ...
    struct FFTDataType {
      using float32    = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftReal, float>;
      using float64    = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftDoubleReal, double>;
      using complex64  = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftComplex, fftwf_complex>;
      using complex128 = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftDoubleComplex, fftw_complex>;
    };

    template <typename ExecutionSpace, typename T>
    struct FFTPlanType {
      using fftwHandle = std::conditional_t<std::is_same_v<KokkosFFT::Impl::real_type_t<T>, float>, fftwf_plan, fftw_plan>;
      using type = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftHandle, fftwHandle>;
    };

    template <typename ExecutionSpace>
    using TransformType = std::conditional_t<std::is_same_v<ExecutionSpace, Kokkos::HIP>, hipfftType, FFTWTransformType>;
    ...
    struct transform_type<ExecutionSpace, T1, Kokkos::complex<T2>> {
      ...
      static constexpr _TransformType m_cuda_type = std::is_same_v<T1, float> ? HIPFFT_R2C : HIPFFT_D2Z;
    ...
    struct transform_type<ExecutionSpace, Kokkos::complex<T1>, T2> {
      ...
      static constexpr _TransformType m_cuda_type = std::is_same_v<T1, float> ? HIPFFT_C2R : HIPFFT_Z2D;
    ...
    struct transform_type<ExecutionSpace, Kokkos::complex<T1>, Kokkos::complex<T2>> {
      ...
      static constexpr _TransformType m_cuda_type = std::is_same_v<T1, float> ? HIPFFT_C2C : HIPFFT_Z2Z;
    ...
    auto direction_type(Direction direction) {
      static constexpr FFTDirectionType _FORWARD = std::is_same_v<ExecutionSpace, Kokkos::HIP> ? HIPFFT_FORWARD : FFTW_FORWARD;
      static constexpr FFTDirectionType _BACKWARD = std::is_same_v<ExecutionSpace, Kokkos::HIP> ? HIPFFT_BACKWARD : FFTW_BACKWARD;
    ...
    struct transform_type<ExecutionSpace, T1, Kokkos::complex<T2>> {
      static_assert(std::is_same_v<T1, T2>, "T1 and T2 should have the same precision");
      using _TransformType = TransformType<ExecutionSpace>;
      static constexpr _TransformType m_type = std::is_same_v<T1, float> ? HIPFFT_R2C : HIPFFT_D2Z;
    ...
    struct transform_type<ExecutionSpace, Kokkos::complex<T1>, T2> {
      static_assert(std::is_same_v<T1, T2>, "T1 and T2 should have the same precision");
      using _TransformType = TransformType<ExecutionSpace>;
      static constexpr _TransformType m_type = std::is_same_v<T2, float> ? HIPFFT_C2R : HIPFFT_Z2D;
      static constexpr _TransformType type() { return m_type; };
    };
    ...
    struct transform_type<ExecutionSpace, Kokkos::complex<T1>, Kokkos::complex<T2>> {
      static_assert(std::is_same_v<T1, T2>, "T1 and T2 should have the same precision");
      using _TransformType = TransformType<ExecutionSpace>;
      static constexpr _TransformType m_type = std::is_same_v<T1, float> ? HIPFFT_C2C : HIPFFT_Z2Z;
      static constexpr _TransformType type() { return m_type; };
    };
